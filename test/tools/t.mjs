...
console.log('verts', w.mesh.count, 'MB pos', (w.mesh.pos.byteLength/1048576).toFixed(1), 'MB col', (w.mesh.col.byteLength/1048576).toFixed(1));
// elevation histogram
const h = {};
...
console.log('path len', w.path.length.toFixed(1), 'lap secs @14u/s', (w.path.length/14).toFixed(0));
// track height continuity
let maxJump=0; for(let i=0;i<1024;i++){ maxJump=Math.max(maxJump, Math.abs(w.path.h[(i+1)%1024]-w.path.h[i])); }
console.log('max per-point height jump', maxJump.toFixed(4), '(x HEIGHT =', (maxJump*HEIGHT).toFixed(4), 'units)');
console.log('track h min/max', Math.min(...w.path.h).toFixed(2), Math.max(...w.path.h).toFixed(2));
// pathAt continuity: sample and check step distance is smooth
let prev=pathAt(w.path,0), md=0;
for(let d=1;d<w.path.length;d+=1){ const p=pathAt(w.path,d); md=Math.max(md, Math.hypot(p.x-prev.x,p.z-prev.z)); prev=p; }
console.log('max pathAt step for 1 unit of arc', md.toFixed(3));
const a=pathAt(w.path,0), b=pathAt(w.path,w.path.length);
